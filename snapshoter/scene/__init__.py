...
def make_folders(keystr, cq_abbr, output_path, prefix=''):
    '''
    ...
    for folder in folders:
        if prefix == '':
            ...
            full_path = os.path.join(
                output_path, keystr, cq_abbr, prefix, folder
            )
...
def copy_all_graphdata_files(keystr, cq_abbr, output_path):
    '''
    Copy all the graphdata files
    '''
    # get the current path of this file
    cur_path = pathlib.Path(__file__).parent.absolute()
    ...
    if os.path.exists(_output_path):
        ...
        print('* removed existing %s' % _output_path)
    shutil.copytree(graphdata_path, _output_path)
    
    print('* copied graphdata %s' % _output_path)
