...
from lnma.analyzer.rpadapter import _meta_trans_metabin, backtransf
from lnma.analyzer.rpadapter import _meta_trans_metagen
from lnma.analyzer.rpadapter import _meta_trans_metacum
from lnma.analyzer.rpadapter import _meta_trans_metaprop, _meta_trans_metarate, _meta_trans_metaprop_pft
...
def analyze_pwma_raw_coe(rs, cfg, has_cumu=True):
    '''
    ...
    ## fix is_hakn missing
    if 'is_hakn' in cfg: pass
    else:
        if 'hakn_adjustment' in cfg:
            cfg['is_hakn'] = cfg['hakn_adjustment']
        else:
            cfg['is_hakn'] = 'FALSE'

    # fix adhoc_hakn
    if 'hakn_adjustment' not in cfg:
        cfg['hakn_adjustment'] = 'FALSE'
    if cfg['hakn_adjustment'] == 'FALSE':
        cfg['adhoc_hakn'] = ''
    else:
        cfg['adhoc_hakn'] = 'se'

    # fix sort_by missing
    if 'sort_by' in cfg: pass
    else: cfg['sort_by'] = 'year'

    # fix imp_t
    if 'imp_t' in cfg:
        try: 
            # if user provides a float value, then ok
            cfg['imp_t'] = float(cfg['imp_t'])
            cfg['is_t_user_provided'] = True
        except:
            cfg['imp_t'] = 0
            cfg['is_t_user_provided'] = False
    else:
        cfg['imp_t'] = 0
        cfg['is_t_user_provided'] = False

    # fix assumed_baseline
    if 'assumed_baseline' in cfg:
        cfg['assumed_baseline'] = int(cfg['assumed_baseline'])
    else:
        cfg['assumed_baseline'] = 100
    ...
    # 2024-06-12: just to ensure the same format
    pma_pooled_effect_rr = pma_pooled_effect
    ci_of_rr = ci_of_sm
    ...
    # last thing, need to merge back the pid and other info to stus
...
def analyze_pwma_pre_coe(rs, cfg):
    '''
    Analyze the primary and cumulative and CoE for pre
    ...
    but Et, Nt, Ec, Nc maybe optional
    '''
    df = pd.DataFrame(rs)

    # the sum, due to Et missing, need to fill 0 first
    df_fillna = df.fillna(0)
    ttEt = df_fillna.Et.sum()
    ttNt = df_fillna.Nt.sum()
    ttEc = df_fillna.Ec.sum()
    ttNc = df_fillna.Nc.sum()
    ...
    # define functions for converting HR to RR and RD
    def _HR2RR(val, ACR): 
        '''Convert HR to RR
        '''
        a = 1 - ACR
    ...
    primma = _meta_trans_metagen(j, cfg)
    # get adjusted effect
    primma_trimfill = _meta_trans_metagen(j_trimfill, cfg)
    ...
    # 2024-06-12: convert HR to RR
    ACR = _get_ACR(ttEc, ttNc)
    pma_pooled_effect_rr = _HR2RR(pma_pooled_effect, ACR)
    ...
    ci_of_rr = [
        _HR2RR(ci_of_sm[0], ACR),
        _HR2RR(ci_of_sm[1], ACR),
    ]
    # get a flag for relative effect
    is_relative_effect_large = bool(pma_pooled_effect_rr < 0.7 or pma_pooled_effect_rr > 1.3)
    ...
    # 2024-06-12: use customized function to get RD
    imp_rd = _HR2RD(
        # HR to RD
        pma_pooled_effect,
        # ACR
    ...
    # get the confidence interval of RD by HR
    ci_of_rd = [
        _HR2RD(ci_of_sm[0], ACR),
        _HR2RD(ci_of_sm[1], ACR)
    ]
    imp_t = cfg['imp_t']
...
def analyze_pwma_incd_pft(rs, p_scale ,cfg , has_cumu=True):
    '''
    Analyze the incidence for PFT
    '''
    # create a dataframe first
    df = pd.DataFrame(rs)
    print("PFT being invoked")
    ...
    ret = {
        ...
        'success': True,
        'data': {
            'incdma': _meta_trans_metaprop_pft(j, cfg, p_scale)
        }
    }
    if has_cumu:
        ret['data']['cumuma'] = _meta_trans_metacum(j, cfg)
...
def analyze_pwma_incd_metarate(rs, cfg, _time, ir_scale , has_cumu=True):
    '''
    Analyze the incidence
    '''
    # create a dataframe first
    ...
    ret = {
        ...
        'success': True,
        'data': {
            'incdma': _meta_trans_metarate(j, cfg, ir_scale)
        }
    }
    if has_cumu:
        ret['data']['cumuma'] = _meta_trans_metacum(j, cfg,)
...
def get_pwma(dataset, datatype='CAT_RAW', 
    sm='RR', method='MH', fixed_or_random='random'):
    '''
    Get the pwma result in a simple way
    '''
    df = pd.DataFrame(
        dataset,
