...
def asin2p(x, n=None, value="mean", warn=False):
    print(x)
    if n==None:
        return x
    ...
    if n is None:
        minimum = np.arcsin(np.sqrt(0))
        maximum = np.arcsin(np.sqrt(1))
    else:
        minimum = 0.5 * (np.arcsin(np.sqrt(0 / (n + 1))) + np.arcsin(np.sqrt((0 + 1) / (n + 1))))
        maximum = 0.5 * (np.arcsin(np.sqrt(n / (n + 1))) + np.arcsin(np.sqrt((n + 1) / (n + 1))))
    ...
    if np.any(sel0):
        if n is None:
            if warn:
                print("Negative value for ", end='')
                if len(x) > 1:
                    print("at least one ", end='')
                if value == "mean":
                    print("transformed proportion using arcsine transformation.\n  Proportion set to 0.")
                elif value == "lower":
                    print("lower confidence limit using arcsine transformation.\n  Lower confidence limit set to 0.")
                elif value == "upper":
                    print("upper confidence limit using arcsine transformation.\n  Upper confidence limit set to 0.")
        else:
            if warn:
                print("Too small value for ", end='')
                if len(x) > 1:
                    print("at least one ", end='')
                if value == "mean":
                    print("transformed proportion using Freeman-Tukey double arcsine transformation.\n  Proportion set to 0.")
                elif value == "lower":
                    print("lower confidence limit using Freeman-Tukey double arcsine transformation.\n  Lower confidence limit set to 0.")
                elif value == "upper":
                    print("upper confidence limit using Freeman-Tukey double arcsine transformation.\n  Upper confidence limit set to 0.")
    
    if np.any(sel1):
        if n is None:
            if warn:
                print("Too large value for ", end='')
                if len(x) > 1:
                    print("at least one ", end='')
                if value == "mean":
                    print("transformed proportion using arcsine transformation.\n  Proportion set to 1.")
    ...
    if n is None:
        ...
        res[sel] = 0.5 * (1 - np.sign(np.cos(2 * x)) * np.sqrt(1 - (np.sin(2 * x) + (np.sin(2 * x) - 1 / np.sin(2 * x)) / n) ** 2))
    
    return res



def backtransf(x, sm, n=None):
    '''
...
def _meta_trans_metaprop_pft(j, params, p_scale):
    '''
    Convert the metaprop result of PWMA Incidence Analysis
    '''
    # it should contain incdma
    ...
    # first, put the basic values
    import statistics
    total_harmonic_mean = statistics.harmonic_mean(data['n'])
    print("Total Harmonic Mean ")
    print(data['n'])
    print(total_harmonic_mean)
    ...
    ret = {
        'model': {
            'random': {
                ...
                'lower': data['lower.random'][0],
                'upper': data['upper.random'][0],
                'bt_TE': _round(backtransf(data['TE.random'][0], sm, sum(data['n'])), 4),
                'bt_lower': _round(backtransf(data['lower.random'][0], sm), 4),
                'bt_upper': _round(backtransf(data['upper.random'][0], sm), 4),
                'bt_ab_TE': _round(backtransf(data['TE.random'][0],sm ,n=total_harmonic_mean) * p_scale, 2),
                'bt_ab_lower': _round(backtransf(data['lower.random'][0], sm, n=total_harmonic_mean) * p_scale, 2),
                'bt_ab_upper': _round(backtransf(data['upper.random'][0], sm, n=total_harmonic_mean) * p_scale, 2),
                'w': 1 
            },
            'fixed': {
                'name': 'Fixed effects model',
    ...
    for i, stu in enumerate(stus):
        ret['stus'].append({
            ...
            'lower': data['lower'][i],
            'upper': data['upper'][i],
            'bt_TE': _round(backtransf(data['TE'][i], sm, n=data['n'][i] ), 4),
            'bt_ab_TE': _round(backtransf(data['TE'][i], sm, n=data['n'][i]) * p_scale, 2),

            # the lower and upper in incidence analysis is already the backtransf parsed
            'bt_lower': _round(data['lower'][i], 4),
            'bt_upper': _round(data['upper'][i], 4),
            'bt_ab_lower': _round(data['lower'][i] * p_scale, 2),
            'bt_ab_upper': _round(data['upper'][i] * p_scale, 2),
...
def _meta_trans_metarate(j, params, ir_scale):
    '''
    Convert the metaprop result of PWMA Incidence Analysis
    '''
    # it should contain incdma
    ...
    ret = {
        'model': {
            'random': {
                ...
                'TE': data['TE.random'][0],
                'seTE': data['seTE.random'][0],
                'lower': _round(backtransf(data['lower.fixed'][0], sm), 4),
                'upper': data['upper.random'][0],
                'bt_TE': _round(backtransf(data['TE.random'][0], sm), 4),
                'bt_lower': _round(backtransf(data['lower.random'][0], sm), 4),
                'bt_upper': _round(backtransf(data['upper.random'][0], sm), 4),
                'bt_ab_TE': _round(backtransf(data['TE.random'][0], sm) * ir_scale, 2),
                'bt_ab_lower': _round(backtransf(data['lower.random'][0], sm) * ir_scale, 2),
                'bt_ab_upper': _round(backtransf(data['upper.random'][0], sm) * ir_scale, 2),
                'w': 1 
            },
            'fixed': {
                'name': 'Fixed effects model',
    ...
    for i, stu in enumerate(stus):
        ret['stus'].append({
            ...
            'TE': data['TE'][i],
            'seTE': data['seTE'][i],
            'lower': _round(backtransf(data['lower'][i],sm), 2),
            'upper': data['upper'][i],
            'bt_TE': _round(backtransf(data['TE'][i], sm), 4),
            'bt_ab_TE': _round(backtransf(data['TE'][i], sm) * ir_scale, 2),

            # the lower and upper in incidence analysis is already the backtransf parsed
            'bt_lower': _round(backtransf(data['lower'][i], sm), 4),
            'bt_upper': _round(data['upper'][i], 4),
            'bt_ab_lower': _round(backtransf(data['lower'][i], sm) * ir_scale, 2),
            'bt_ab_upper': _round(data['upper'][i] * ir_scale, 2),
