...
def get_pwma(extract, paper_dict, is_skip_unselected=True):
    '''
    Get the PWMA result in the given extract with paper dict

    The pwma result may be complex depends on the given input
    ...
    if input_format not in settings.INPUT_FORMAT_TPL['pwma']:
        # what??? how could it be??
        print('* unknown input_format %s for PWMA' % input_format)
        return None
    ...
    return pwma(
        rscfg,
        extract.meta['group'] == 'subgroup'
    )
    
    
def pwma(rscfg, is_subg_analysis):
    '''
    The raw pwma without Extract

    rscfg is a dictionary: 
    {
        "rs": [],
        "cfg": {}
    }
    ...
    # 1. add the pid to the PWMA result stus
