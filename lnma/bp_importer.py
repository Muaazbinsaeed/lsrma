...
@bp.route('/by_user_input_pmid')
@login_required
def by_user_input_pmid():
    return render_template('importer/by_user_input_pmid.html', ss=ss_state)


@bp.route('/by_user_input_doi')
@login_required
def by_user_input_doi():
    return render_template('importer/by_user_input_doi.html', ss=ss_state)


@bp.route('/by_user_input_manually')
@login_required
def by_user_input_manually():
    return render_template('importer/by_user_input_manually.html', ss=ss_state)
...
@bp.route('/import_one_doi', methods=['POST'])
@login_required
def import_one_doi():
    ...
    is_success, paper = srv_import.import_by_doi(
        project_id,
        doi,
        nct
    )
...
@bp.route('/upload_user_input_manually', methods=['GET', 'POST'])
@login_required
def upload_user_input_manually():
    if request.method == 'GET':
        return ''

    # first, check pid to decide what to do next
    pid = request.form.get('pid').strip().lower()
    pid_type = request.form.get('pid_type').strip()
