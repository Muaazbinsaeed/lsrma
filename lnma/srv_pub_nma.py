...
def get_graph_nma_json(keystr, cq_abbr):
    '''
    Make GRAPH_NMA.json
    '''
    # get the cache path
    ...
    ret = get_graph_nma_data_from_db(
        keystr, 
        cq_abbr
    )
...
def get_softable_nma_json(keystr, cq_abbr):
    '''
    Make SOFTABLE_NMA.json
    '''
...
def get_evmap_json(keystr, cq_abbr):
    '''
    Make EVMAP.json
    '''
    fn_json = 'EVMAP.json'
    ...
    full_fn_softable_nma = os.path.join(
        full_path,
        'SOFTABLE_NMA.json'
    )
    print('* using %s.%s softable nma cache %s' % (
        keystr, cq_abbr, full_fn_softable_nma
    ))
    j = json.load(open(full_fn_softable_nma))
    ret = parse_evmap_data_from_json(j)
    latest = srv_project.get_project_latest_stat_by_keystr(keystr)
    ret['latest'] = latest
...
def get_graph_nma_data_from_db(keystr, cq_abbr):
    ...
    for ext_idx, extract in enumerate(extracts):
        ...
        if extract.meta['included_in_plots'] != 'yes':
            ...
            print('* skip extract %s due to included_in_plots=%s' % (
                extract.abbr,
                extract.meta['included_in_plots']
            ))
            continue
    ...
    if len(failed_extracts) == 0:
        ...
        for idx, err_ext in enumerate(failed_extracts):
            ...
            print('* %s' % err)
            print('')
            updated_failed_extracts.append({
                ext.get_repr_str() : err
            })
    ret['failed_extracts']= updated_failed_extracts
...
def get_nma_by_cq(keystr, cq_abbr="default", included_in='sof'):
    ...
    for ext_idx, extract in enumerate(extracts):
        ...
        # check if included in sof
        if extract.meta['included_in_%s' % included_in] != 'yes':
            # this oc is NOT included
            print('* skip due to included_in_%s=%s' % (
                included_in,
                extract.meta['included_in_%s' % included_in]
            ))
            continue

        # treatments = extract.get_treatments_in_data()
        # 2023-06-19: fix data bug
        extract = dora.attach_extract_data(extract)
        ...
        print('* converted NMA result to league table')
        ...
        print('* converted NMA result to rank table')

        # get the cetable?
        cetable = {}
        if 'cetable' in extract.meta:
            cetable = extract.meta['cetable']
            print('* found cetable of %s cmps in extract.meta' % (
                len(cetable.keys())
            ))
            util.print_cetable(cetable)
        else:
            print('* NOT found cetable in extract.meta')
        ...
        print('* got trtable from NMA data %s treatments: %s' % (
            len(trtable.keys()),
            ', '.join(trtable.keys())
        ))
        ...
        print('* created oc_dict item for %s' % (
            extract.get_repr_str()
        ))
...
def _add_cie_patch(keystr, cq_abbr, oc_dict):
    ...
    with open(full_fn, 'r', encoding='utf-8-sig') as csv_file:
        ...
        print('* loaded the CoE Path file %s' % (
            full_fn
        ))
