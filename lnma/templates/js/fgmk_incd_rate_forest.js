var fgmk_incd_rate_forest = {
    ...
    return {
        ...
        cols: [
            ...
            {width: 50,  align: 'end',    name: 'Time', row: 2},
            {width: 100,  align: 'end',    name: 'Incidence Rate', row: 2},
            {width: 135, align: 'end',    name: '95% CI', row: 2},
            {width: 200, align: 'middle', name: 'Incidence per', row: 2},
        ...
        draw: function(data, cfg, pId_dict, nct_dict) {
            var irType = document.getElementById("irtype-type");
            var filter23 = document.getElementById("filter23");
            this.cols[6].name = 'Incidence per ' + (irType ? irType.value + ' ' : 10 + ' ') + (filter23 ?      `${filter23.value}` : `/nPerson-Months`);
        ...
        _draw_forest: function() {
           ...
           for (i=0; i<this.data.stus.length; i++){
            ...
            this.xAxis = d3.axisBottom(this.x_scale)
                ...
                .tickValues(tickValues_); // Explicitly set tick values if you want specific points
            this.svg.append('g')
        ...
        get_txt_by_col: function(obj, idx) {
            var lower = obj.bt_ab_lower>0 ?obj.bt_ab_lower.toFixed(2) : 0.00.toFixed(2)
            is_float = Number(obj.N) === obj.N && obj.N % 1 !== 0;
            var total_n = is_float?obj.N.toFixed(2):obj.N;
            switch(idx) {
                case 0: return obj.name;
                case 2: return obj.E;
                case 3: return total_n;
                case 4: return obj.bt_ab_TE.toFixed(2);
                case 5: return '['+lower+'; '+obj.bt_ab_upper.toFixed(2)+']';
                case 6: return ''; 
                
                // case 6 is the figure, so no text
