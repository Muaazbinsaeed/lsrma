var srv_screener = {
    ...
    api_url: {
        ...
        find_duplicates: '[[ url_for("screener.find_duplicates") ]]',
        keep_and_exclude: '[[ url_for("screener.keep_and_exclude") ]]',
        ...
        set_pid: '[[ url_for("screener.set_pid") ]]',
    ...
    get_graphical_decision_ss(code){
        if (this.ss.graphical_decision_data.hasOwnProperty(code)) {
            return this.ss.graphical_decision_data[code];
    ...
    get_graphical_import_ss(code){
        if (this.ss.graphical_import_data.hasOwnProperty(code)) {
            return this.ss.graphical_import_data[code];
    ...
    find_duplicates: function(src, callback) {
        var project_id = Cookies.get('project_id');

        $.ajax({
            type: 'GET',
            dataType: "json",
            url: this.api_url.find_duplicates,
            data: {
                project_id: project_id,
                src: src
            },
            ...
            success: callback,
            error: function(jqXHR, textStatus, errorThrown) {
                jarvis.toast('Something wrong when finding deduplicates. Refresh page and try later.', 'error');
                console.error(textStatus, errorThrown);
            }
        });
    },

    keep_and_exclude: function(pid_to_keep, dup_pids, callback) {
        var project_id = Cookies.get('project_id');

        $.ajax({
            type: 'POST',
            dataType: "json",
            url: this.api_url.keep_and_exclude,
            data: {
                project_id: project_id,
                pid_to_keep: pid_to_keep,
                dup_pids: dup_pids.join(',')
            },
            cache: false,
            success: callback,
            error: function(jqXHR, textStatus, errorThrown) {
                jarvis.toast('Something wrong when keeping and excluding paper', 'alert');
                console.error(textStatus, errorThrown);
            }
        });
    },
     ...
     * @param {Function} callback 
     */
    set_pid: function(paper_id, pid, callback) {
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: this.api_url.set_pid,
            data: {
                paper_id: paper_id,
                pid: pid
            },
            ...
            success: callback,
            error: function(jqXHR, textStatus, errorThrown) {
                jarvis.toast('Something wrong when setting new pid for paper', 'alert');
                console.error(textStatus, errorThrown);
            }
        });
    },
